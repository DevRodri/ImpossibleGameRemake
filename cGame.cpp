...
bool cGame::ManageLogic()
{
	...
	case STATE_GAME:
		...
		else if (ly > CAMERA_MIN_Y)
		{
			...
			if ((gy - d0) > 512) 
			{ //esta casi fuera de la pantalla dios mio!!!!
				Scene.SetOffsetYCamera(offsetYCam - (gy-d0-512)); //lo resituo dentro de pantalla
			}
			else Scene.SetOffsetYCamera(offsetYCam - 4);
		}
		Scene.GetOffsetYCamera(&offsetYCam);
		d0 = (SCENE_AREA_X * 32) - SCREEN_RES_Y - offsetYCam + 56;
